/**
 * Generates src/icons/wordpress.ts and src/icons/mui.ts by rendering each
 * icon to an SVG string at build time, so the browser bundle never imports
 * the raw icon packages or calls renderToString at runtime.
 *
...
function writeTs( filePath, exportName, entries ) {
	...
	const src  = [
		...
		`export const ${ exportName }: IconEntry[] = ${ json };`,
	].join( '\n' ) + '\n';
...
writeTs( path.join( outDir, 'wordpress.ts' ), 'wordpressIcons', wordpressIcons );
console.timeEnd( 'wordpress' );
console.log( `  → ${ wordpressIcons.length } WordPress icons written` );
...
writeTs( path.join( outDir, 'mui.ts' ), 'muiIcons', muiMaterialIcons );
console.timeEnd( 'mui' );
console.log( `  → ${ muiMaterialIcons.length } MUI icons written` );
